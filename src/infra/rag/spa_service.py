...
Reads configuration from environment variables and produces idempotent
Kubernetes manifests.  Sensitive values are never written to YAML; they
are applied directly to the cluster as a Secret.

Usage:
    python frontend_service.py --write      # only write manifests to disk
    python frontend_service.py --apply      # write and apply to cluster
    python frontend_service.py --apply-secrets  # apply secrets only
    python frontend_service.py --delete     # delete manifests and secret
"""
...
# ---------------------------------------------------------------------------
# Defaults – strong production values, all can be overridden via environment
# ---------------------------------------------------------------------------
...
# Keys considered sensitive – will be stored in K8s Secret
SECRET_KEYS = (
    "SESSION_SECRET",
    "VALKEY_PASSWORD",
    "GOOGLE_CLIENT_ID",
...
# Keys that should never be written to disk even as YAML
...
def load_config() -> dict[str, Any]:
    ...
    cfg["METRICS_PORT"] = _env_int("METRICS_PORT", DEFAULTS["METRICS_PORT"])
...
def collect_secret_env() -> dict[str, str]:
    """Collect sensitive values from environment. Ensure keys are non‑empty."""
    ...
    for key in SECRET_KEYS:
        ...
        if not v:
            continue
        secret_env[key] = v
    # Also include VALKEY URL only if a password is set; otherwise it's not a secret
    if "VALKEY_PASSWORD" in secret_env:
        # The actual VALKEY_URL will be constructed inside the pod using the secret
        pass
    return secret_env
...
def apply_secret_direct(cfg: dict[str, Any], secret_env: dict[str, str]) -> None:
    """Apply secrets directly via kubectl (avoid writing to YAML)."""
    if not secret_env:
        log.info("No secret values provided; skipping.")
        return
    cmd = [
...
def generate_manifests(
    ...
    """
    Generate manifests on disk.  Only non‑AWS secrets are written to YAML;
    AWS keys are never written (they are applied directly if needed).
    """
    manifests_dir: Path = cfg["MANIFESTS_DIR"]
    ensure_dir(manifests_dir)

    # Split secrets: safe to YAML vs. direct apply
    secret_for_yaml: dict[str, str] = {}
    secret_for_direct: dict[str, str] = {}
    for k, v in secret_env.items():
        if k in AWS_SECRET_KEYS:
            secret_for_direct[k] = v
        else:
            secret_for_yaml[k] = v
    ...
    sec_doc = build_secret_doc(cfg, secret_for_yaml)
...
def apply_to_cluster(
    ...
    # Apply direct secrets first
    if secret_for_direct:
        try:
            create_namespace_if_missing(cfg["NAMESPACE"])
            apply_secret_direct(cfg, secret_for_direct)
...
def parse_args(argv: list[str] | None = None) -> Any:
    p = argparse.ArgumentParser(description="Generate and manage Frontend + Auth manifests.")
    group = p.add_mutually_exclusive_group(required=True)
    group.add_argument("--apply-secrets", action="store_true", help="Create/update secrets in-cluster (no YAML).")
