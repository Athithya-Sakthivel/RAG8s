...
Usage:
    python3 src/infra/rag/spa_service.py --apply-secrets   # apply secrets only
    python3 src/infra/rag/spa_service.py --write           # only write manifests to disk
    python3 src/infra/rag/spa_service.py --apply           # write and apply to cluster
    python3 src/infra/rag/spa_service.py --delete          # delete manifests and secret
"""
...
# Keys considered sensitive – stored in K8s Secret
SECRET_KEYS = (
    "SESSION_SECRET",
    "VALKEY_PASSWORD",
    "GOOGLE_CLIENT_ID",
...
# Keys that are auto-generated by the generator if not present
...
def _get_existing_secret_data(namespace: str, name: str) -> dict[str, str] | None:
    """Fetch existing secret data from the cluster, or None if not found."""
    try:
        proc = subprocess.run(
...
def _auto_generate_missing_keys(secret_env: dict[str, str], namespace: str, secret_name: str) -> None:
    """
    Ensure SESSION_SECRET and JWT_PRIVATE_KEY_PEM exist in secret_env.
    If not provided, look them up in the existing cluster secret.
    If still missing, generate new ones and add them.
    """
    ...
    if "SESSION_SECRET" not in secret_env or not secret_env["SESSION_SECRET"]:
        if "SESSION_SECRET" in existing:
            # value is base64 encoded in the secret data, decode it
...
def collect_secret_env() -> dict[str, str]:
    """Collect user-provided secret values from environment. Auto-generated
    keys will be filled later."""
    secret_env: dict[str, str] = {}
    # Only collect keys that are explicitly set in the environment
    for key in SECRET_KEYS:
        ...
        if not v:
            continue
        secret_env[key] = v
    # AWS keys are collected separately if USE_IAM=false,
    # but they are not part of SECRET_KEYS; they will be added later.
    return secret_env

def _aws_cred_env() -> dict[str, str]:
    """Return dict of AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY if set."""
    creds = {}
...
def build_secret_doc(cfg: dict[str, Any], secret_env: dict[str, str]) -> dict[str, Any] | None:
    """Build a Secret manifest for non‑AWS, user-provided keys written to disk."""
    if not secret_env:
        return None
    return {
        "apiVersion": "v1",
...
def build_deployment_doc(cfg: dict[str, Any], has_secret: bool) -> dict[str, Any]:
    ...
    env_vars: list[dict[str, Any]] = [
        ...
        # IAM
        {"name": "USE_IAM", "value": "true" if use_iam else "false"},
    ]

    if use_iam and cfg["IRSA_ROLE_ARN"]:
        env_vars.append({"name": "IRSA_ROLE_ARN", "value": cfg["IRSA_ROLE_ARN"]})

    # If a secret is provided, mount all non-AWS keys as envFrom
    ...
    # automountServiceAccountToken must be true for IRSA
    automount_token = use_iam
...
def _apply_secret_direct(cfg: dict[str, Any], secret_data: dict[str, str]) -> None:
    """Apply secret directly to cluster, handling JWT_PRIVATE_KEY_PEM via file."""
    if not secret_data:
...
def _delete_secret_direct(cfg: dict[str, Any]) -> None:
    subprocess.run(
...
def generate_manifests(
    ...
    Only non-AWS secrets are written to YAML; AWS keys are handled later.
    """
    manifests_dir: Path = cfg["MANIFESTS_DIR"]
    ensure_dir(manifests_dir)

    # Split secrets: what goes into the YAML secret (non-AWS, non-IAM condition)
    use_iam = bool(cfg["USE_IAM"])
    secret_for_yaml: dict[str, str] = {}
    # AWS creds are not written to disk; they are only applied if USE_IAM=false
    for k, v in secret_env.items():
        if k in AWS_CRED_KEYS:
            continue  # handled separately in apply phase
        secret_for_yaml[k] = v

    has_secret = bool(secret_for_yaml)
    secret_keys_hash = sha256_secret_keys(secret_for_yaml)  # only for YAML keys
    docs_for_hash = [
...
def apply_to_cluster(
    ...
    # 1. Auto-generate idempotent secrets (SESSION_SECRET, JWT_PRIVATE_KEY_PEM)
    _auto_generate_missing_keys(secret_env, cfg["NAMESPACE"], cfg["SECRET_NAME"])

    # 2. Collect AWS credentials if needed
    aws_creds = _aws_cred_env() if not use_iam else {}

    # 3. Combine user-provided + auto-generated + AWS (if any) into final secret
    final_secret = dict(secret_env)
    final_secret.update(aws_creds)

    # 4. Generate manifests on disk (does not include AWS creds in YAML)
    generate_manifests(cfg, secret_env, dry_run=dry_run, verbose=verbose)  # pass original secret_env (without AWS)
    if dry_run:
        log.info("Dry-run requested; skipping apply actions.")
        return

    # 5. Apply the full secret to cluster (including AWS creds if USE_IAM=false)
    if final_secret:
        try:
            create_namespace_if_missing(cfg["NAMESPACE"])
            _apply_secret_direct(cfg, final_secret)
    ...
    try:
        ...
        # The 02-secret.yaml may not contain all keys (only non-AWS), but we apply anyway for idempotency
    ...
    log.info("Manifests and secrets applied.")

def apply_secrets_only(cfg: dict[str, Any], dry_run: bool = False) -> int:
    """Apply only the secret to the cluster (for --apply-secrets)."""
    if not shutil.which("kubectl"):
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.write:
            # Only write manifests; secrets are not applied
            secret_env = collect_secret_env()
