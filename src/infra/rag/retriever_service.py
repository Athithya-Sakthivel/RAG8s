...
DEFAULTS: dict[str, Any] = {
    ...
    "IMAGE": "ghcr.io/athithya-sakthivel/retriever:2026-05-08-13-23--6141057@sha256:fa056fd6d0233fac2f92308fba1695e550af5d87b0fe53276bf2c0e70b96563b",
...
def _base_labels(cfg: dict[str, Any]) -> dict[str, str]:
    return {"app.kubernetes.io/name": cfg["SERVICE_NAME"], "app.kubernetes.io/instance": cfg["DEPLOYMENT_NAME"], "app.kubernetes.io/managed-by": "retriever-manifest-generator", "app.kubernetes.io/component": "retriever"}
...
def _probe_http(path: str, port: int, initial_delay: int, period: int, timeout: int, failure_threshold: int) -> dict[str, Any]:
    return {"httpGet": {"path": path, "port": port, "scheme": "HTTP"}, "initialDelaySeconds": initial_delay, "timeoutSeconds": timeout, "periodSeconds": period, "failureThreshold": failure_threshold, "successThreshold": 1}
...
def build_service_account_doc(cfg: dict[str, Any]) -> dict[str, Any]:
    metadata: dict[str, Any] = {"name": cfg["SERVICE_ACCOUNT_NAME"], "namespace": cfg["NAMESPACE"], "labels": _base_labels(cfg)}
...
def build_secret_doc(cfg: dict[str, Any], secret_env_for_yaml: dict[str, str]) -> dict[str, Any] | None:
    if not secret_env_for_yaml:
        return None
    return {"apiVersion": "v1", "kind": "Secret", "metadata": {"name": cfg["SECRET_NAME"], "namespace": cfg["NAMESPACE"], "labels": _base_labels(cfg)}, "type": "Opaque", "stringData": secret_env_for_yaml}
def build_deployment_doc(cfg: dict[str, Any], has_secret: bool) -> dict[str, Any]:
    ...
    env_vars = [
        ...
        {"name": "ENABLE_PRESIGNED_URLS", "value": str(cfg["ENABLE_PRESIGNED_URLS"]).lower()},
        {"name": "PRESIGNED_URL_TTL_SECONDS", "value": str(cfg["PRESIGNED_URL_TTL_SECONDS"])},
    ]
    ...
    container = {
        ...
        "env": env_vars,
        "envFrom": env_from,
        "volumeMounts": [{"name": "tmp", "mountPath": "/tmp"}, {"name": "tmp", "mountPath": "/var/tmp"}],
        "securityContext": {"allowPrivilegeEscalation": bool(cfg["ALLOW_PRIV_ESC"]), "readOnlyRootFilesystem": bool(cfg["READONLY_ROOTFS"]), "runAsNonRoot": bool(cfg["RUN_AS_NONROOT"]), "runAsUser": int(cfg["RUN_AS_USER"]), "runAsGroup": int(cfg["RUN_AS_GROUP"])},
    ...
    return {"apiVersion": "apps/v1", "kind": "Deployment", "metadata": {"name": cfg["DEPLOYMENT_NAME"], "namespace": cfg["NAMESPACE"], "labels": _base_labels(cfg)}, "spec": {"replicas": cfg["REPLICAS"], "revisionHistoryLimit": 3, "selector": {"matchLabels": {"app.kubernetes.io/name": cfg["SERVICE_NAME"], "app.kubernetes.io/instance": cfg["DEPLOYMENT_NAME"], "app.kubernetes.io/component": "retriever"}}, "template": {"metadata": {"labels": _pod_labels(cfg)}, "spec": {"serviceAccountName": cfg["SERVICE_ACCOUNT_NAME"], "automountServiceAccountToken": True, "terminationGracePeriodSeconds": 30, "securityContext": {"fsGroup": cfg["FS_GROUP"]}, "volumes": [{"name": "tmp", "emptyDir": {}}], "containers": [container]}}}}
def build_service_doc(cfg: dict[str, Any]) -> dict[str, Any]:
    ...
    if cfg["ENABLE_PROMETHEUS"] and cfg["PROMETHEUS_PORT"] != cfg["CONTAINER_PORT"]:
        ports.append({"name": "metrics", "port": cfg["PROMETHEUS_PORT"], "targetPort": cfg["PROMETHEUS_PORT"], "protocol": "TCP"})
    return {"apiVersion": "v1", "kind": "Service", "metadata": {"name": cfg["SERVICE_NAME"], "namespace": cfg["NAMESPACE"], "labels": _base_labels(cfg)}, "spec": {"type": cfg["SERVICE_TYPE"], "selector": {"app.kubernetes.io/name": cfg["SERVICE_NAME"], "app.kubernetes.io/instance": cfg["DEPLOYMENT_NAME"], "app.kubernetes.io/component": "retriever"}, "ports": ports}}
...
def delete_secret_direct(cfg: dict[str, Any]) -> None:
    subprocess.run(["kubectl", "delete", "secret", cfg["SECRET_NAME"], "-n", cfg["NAMESPACE"], "--ignore-not-found"], check=False, capture_output=True)
    log.info("Deleted secret if it existed")
def _extract_image_from_yaml(path: Path) -> str | None:
    try:
...
def generate_manifests(cfg: dict[str, Any], secret_env: dict[str, str], dry_run: bool = False, verbose: bool = False) -> str | None:
    ...
    if inputs_path.exists():
        try:
            ...
            existing = ""
    deployment_path = cfg["FILES"]["deployment"]
    existing_deployment_image = None
    if deployment_path.exists():
        existing_deployment_image = _extract_image_from_yaml(deployment_path)
    if existing == inputs_hash and not dry_run:
        if existing_deployment_image and existing_deployment_image != cfg["IMAGE"]:
            log.info("Detected image mismatch: file image=%s cfg image=%s; regenerating manifests", existing_deployment_image, cfg["IMAGE"])
        else:
...
def parse_args(argv: list[str] | None = None) -> Any:
    ...
    p.add_argument("--dry-run", action="store_true", help="Do not apply anything to cluster; only generate files.")
