...
# --- Manifest Rendering (ZERO SYNTAX ERRORS) ---
render_manifest() {
  log_step "Rendering Valkey StatefulSet manifest"
  mkdir -p "$(dirname "${CLUSTER_FILE}")"

  # Build the manifest using a single heredoc with conditional includes
...
EOF
  ...
  # Add persistence volume mounts if enabled
...
EOF
  ...
  # Add volumeClaimTemplates block if persistence enabled
  if [[ "${ENABLE_PERSISTENCE}" == "1" ]]; then
    cat >> "${CLUSTER_FILE}" <<EOF
...
EOF
  ...
  if ! kubectl apply --dry-run=client -f "${CLUSTER_FILE}" >/dev/null 2>&1; then
    log_error "Manifest validation failed. Attempting to show error:"
    kubectl apply --dry-run=client -f "${CLUSTER_FILE}" 2>&1 || true
...
delete_resources() {
  ...
  kubectl -n "${NAMESPACE}" delete networkpolicy valkey-allow-same-namespace --ignore-not-found >/dev/null 2>&1 || true
