...
# ---------------------------------------------------------------------------
# Prometheus metrics (replaces OTEL)
# ---------------------------------------------------------------------------
_PIPELINE_DURATION = Histogram(
    "retrieval_pipeline_duration_seconds",
    "Total pipeline duration",
    ("outcome", "deployment_environment", "env"),
)
_QDRANT_QUERY_COUNT = Counter(
    "retrieval_qdrant_query_total",
    "Qdrant query count",
    ("mode", "deployment_environment", "env"),
)
_QDRANT_QUERY_LATENCY = Histogram(
    "retrieval_qdrant_query_duration_seconds",
    "Qdrant query latency",
    ("mode", "deployment_environment", "env"),
)
_CACHE_LOOKUP_COUNT = Counter(
    "retrieval_cache_lookup_total",
    "Cache lookup count",
    ("result", "deployment_environment", "env"),
)
_CACHE_LOOKUP_LATENCY = Histogram(
    "retrieval_cache_lookup_duration_seconds",
    "Cache lookup latency",
    ("result", "deployment_environment", "env"),
)
_CACHE_WRITE_COUNT = Counter(
    "retrieval_cache_write_total",
    "Cache write count",
    ("result", "cache_kind", "deployment_environment", "env"),
)
_CACHE_WRITE_LATENCY = Histogram(
    "retrieval_cache_write_duration_seconds",
    "Cache write latency",
    ("cache_kind", "deployment_environment", "env"),
)
_SERVICE_READY = Gauge("retrieval_service_ready", "Service readiness (1=ready)", ("deployment_environment", "env"))
...
def initialize_pipeline_metrics() -> None:
    """No‑op kept for compatibility; Prometheus metrics are registered at import."""
...
def _set_ready(value: bool) -> None:
    ...
    _SERVICE_READY.labels(deployment_environment=DEPLOYMENT_ENVIRONMENT, env=ENV).set(_READY_VALUE)
...
def _resolve_tenant_id(tenant_id: str | None) -> str | None:
    resolved = (tenant_id or TENANT_ID or "").strip()
    ...
    *,
    ...
    return PipelineResult(
        ...
        cache={
            ...
            "id": cache_resp.get("cache_id") or cache_id,
        },
        cache_hit=True,
    ...
    *,
    ...
    try:
        ok = await state.store.semantic_cache_upsert(
            ...
            query_vector=pipeline.dense_vector,
            query_text=pipeline.query_text or "",
            query_norm=pipeline.query_norm or "",
            corpus_version=pipeline.corpus_version or "",
            prompt_version=pipeline.prompt_version or "",
