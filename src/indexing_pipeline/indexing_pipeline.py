        ...
        try:
            for line in iter(stream.readline, ""):
                if not line:
                    break
                text = line.rstrip("\n")
                slog("warning" if is_err else "info", "subprocess.line", text, script=prefix, stream="stderr" if is_err else "stdout", line=text)
        except Exception as e:
...
def invoke_backup(workdir: str) -> None:
    ...
    for attempt in range(1, retries + 1):
        slog("info", "backup.invoke", "Invoking backup script", attempt=attempt, retries=retries, script=backup_script, bucket=s3_bucket, prefix=s3_prefix)
        ...
        if rc == 0:
            ...
            return
        last_err = (rc, out, err)
        slog("warning", "backup.failed_attempt", "Backup attempt failed", attempt=attempt, rc=rc, stdout=(out[-200:] if out else ""), stderr=(err[-200:] if err else ""))
        if attempt < retries:
            _sleep_with_backoff(base, attempt)
    rc, out, err = last_err if last_err else (3, "", "unknown error")
    note_issue("error", "backup.failed", "Backup failed after retries", code=rc or 3, rc=rc, stdout=(out[:2000] if out else ""), stderr=(err[:2000] if err else ""))
...
def run_pipeline(workdir: str) -> None:
    ...
    ok = run_pre_conversions(workdir)
    if not ok:
        note_issue("error", "preconversions.halted", "Pre conversions failed; aborting pipeline", code=1)
        return
    ...
    index_tail = _env_int("INDEX_STDOUT_TAIL_LINES", 2000)
    rc, stdout_lines, stderr_lines = run_local_and_capture(index_path, workdir, timeout=index_timeout, max_lines=index_tail)
    if rc != 0:
        note_issue("error", "index.failed", "Index failed", code=rc, rc=rc, stdout=(stdout_lines[-1] if stdout_lines else ""), stderr=(stderr_lines[-1] if stderr_lines else ""))
        return
