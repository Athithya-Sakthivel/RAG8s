...
def retry_call(
    ...
    if last is not None:
        raise RuntimeError("retry_call failed") from last
...
def _build_rows_from_sections(
    ...
    for sec in sections:
        ...
        def emit_chunk(
            ...
            token_count: int,
            *,
            _heading_path: tuple[str, ...] = tuple(heading_path),
            ...
            sections = [
                {
                    ...
                    "level": 0,
                    "start_line": 0,
                    "end_line": len(canonical_full.splitlines()),
                    "lines": canonical_full.splitlines(),
                }
            ]

        source_url = f"s3://{DATA_S3_BUCKET}/{s3_key}"
        file_name = _file_name_from_source(_safe_str(manifest.get("source_url"), ""), s3_key)
        rows = _build_rows_from_sections(
            doc_id=doc_id,
            raw_key=s3_key,
            file_name=file_name,
            source_url=_safe_str(manifest.get("source_url"), source_url),
            original_manifest=_sanitize_manifest(manifest or {}),
            sections=sections,
        )
...
def main() -> int:
    log("info", "startup", "Starting markdown parser", bucket=DATA_S3_BUCKET, region=AWS_REGION)
